...
@section('content')
<div class="p-6 bg-white rounded-xl shadow-md" x-data="questionManager({{ $survey->questions->pluck('id') }})">
    ...
    <div class="mb-8 bg-white rounded-xl p-4 md:p-6 border-l-4 border-teal-500 shadow">
        <div class="flex flex-col md:flex-row md:justify-between md:items-start">
            <div>
                <nav class="text-sm mb-2 font-medium text-gray-500" aria-label="Breadcrumb">
                    <ol class="list-none p-0 inline-flex">
                        ...
                        <li class="flex items-center"><a href="{{ route('unitkerja.admin.surveys.index') }}" class="hover:text-teal-600">Manajemen Survei</a><svg class="fill-current w-3 h-3 mx-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512">
                                <path d="M285.476 272.971L91.132 467.314c-9.373 9.373-24.569 9.373-33.941 0l-22.667-22.667c-9.357-9.357-9.375-24.522-.04-33.901L188.505 256 34.484 101.255c-9.335-9.379-9.317-24.544.04-33.901l22.667-22.667c9.373-9.373 24.569 9.373 33.941 0L285.475 239.03c9.373 9.372 9.373 24.568.001 33.941z" />
                            </svg></li>
            ...
            <div class="mt-4 md:mt-0 flex flex-col md:flex-row items-stretch md:items-center gap-3 self-start md:self-end">
                <a href="{{ route('unitkerja.admin.surveys.index') }}" @click="pageLoading = true" class="bg-white text-gray-700 px-4 py-2 rounded-lg font-medium hover:bg-gray-100 transition flex items-center justify-center space-x-2 border border-gray-300">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
                    <span>Kembali</span>
                </a>
                <button x-show="isOrderChanged" x-transition @click="saveOrder('{{ route('surveys.questions.reorder', $survey) }}')" class="bg-green-500 text-white px-4 py-2 rounded-lg font-medium hover:bg-green-600 transition shadow-md flex items-center justify-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                    </svg>
                    <span>Simpan Urutan</span>
                </button>
                <a href="{{ route('surveys.questions.create', $survey->id) }}" @click="pageLoading = true" class="bg-teal-500 text-white px-4 py-2 rounded-lg font-medium hover:bg-teal-600 transition shadow-md flex items-center justify-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div>
        ...
        <div x-ref="sortableContainer" class="space-y-3">
            ...
            <div data-id="{{ $question->id }}" class="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-lg hover:border-teal-300 transition-all duration-300 group">
                <div @click="openAccordion = (openAccordion === {{ $question->id }} ? null : {{ $question->id }})" class="flex items-center p-4 cursor-pointer">
                    ...
                    <div class="flex-grow">
                        <p class="font-semibold text-gray-800 group-hover:text-teal-600 transition-colors">{{ $loop->iteration }}. {{ $question->question_body }}</p>
                    </div>
                    ...
                    <div class="flex items-center space-x-3 flex-shrink-0 ml-4">
                        <a href="{{ route('surveys.questions.edit', ['survey' => $survey->id, 'question' => $question->id]) }}" @click="pageLoading = true" class="text-teal-600 hover:text-teal-800 opacity-0 group-hover:opacity-100 transition-opacity" title="Edit Pertanyaan">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                        </a>
                        <button type="button" @click.stop="$dispatch('open-delete-modal', { url: '{{ route('surveys.questions.destroy', [$survey, $question]) }}', name: '{{ addslashes($question->question_body) }}' })" class="text-red-600 hover:text-red-800 opacity-0 group-hover:opacity-100 transition-opacity" title="Hapus Pertanyaan">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="border-t pt-4">
                        ...
                        <ul class="space-y-2 text-sm text-gray-700">
                            ...
                            <li class="flex items-center gap-2">
                                ...
                                <span>{{ $option->option_body }} (Skor: <span class="font-semibold text-teal-700">{{ $option->option_score }}</span>)</span>
                            </li>
                            @endforeach
                        </ul>
                    </div>
