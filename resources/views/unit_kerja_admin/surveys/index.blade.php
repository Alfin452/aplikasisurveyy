        ...
        <form action="{{ route('unitkerja.admin.surveys.index') }}" method="GET">
            <div class="flex flex-wrap items-center gap-4">
                ...
                <div class="flex-grow relative min-w-[250px]">
                    ...
                    <input type="text" name="search" class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="Cari judul survei..." value="{{ request('search') }}">
                </div>

                {{-- Dropdown Filter --}}
                <select name="status" class="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-teal-500">
                    <option value="">Semua Status</option>
                    <option value="1" {{ request('status') == '1' ? 'selected' : '' }}>Aktif</option>
                    <option value="0" {{ request('status') === '0' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
                <select name="year" class="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-teal-500">
                    <option value="">Semua Tahun</option>
                    @foreach ($years as $year)
                    <option value="{{ $year }}" {{ request('year') == $year ? 'selected' : '' }}>{{ $year }}</option>
                    @endforeach
                </select>
                <select name="sort" class="w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-teal-500">
                ...
                <div class="flex items-center gap-3 ml-auto">
                    <a href="{{ route('unitkerja.admin.surveys.index') }}" class="text-sm font-semibold text-gray-600 hover:text-gray-800">Reset</a>
                    <button type="submit" class="bg-teal-500 text-white px-5 py-2 rounded-lg font-semibold hover:bg-teal-600 transition shadow-sm flex items-center space-x-2">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium align-top">
                        <div class="flex items-center justify-center space-x-4">
                            ...
                            <a href="{{ route('unitkerja.admin.surveys.edit', $survey->id) }}" class="text-teal-600 hover:text-teal-800" title="Edit Survei">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                            </a>
                            <button type="button"
                                @click="openDeleteModal = true; deleteUrl = '{{ route('unitkerja.admin.surveys.destroy', $survey->id) }}'; deleteItemName = '{{ addslashes($survey->title) }}'"
                ...
                <tr>
                    <td colspan="4" class="text-center text-gray-500 py-6">
                        Tidak ada survei yang ditemukan untuk unit kerja Anda.
                    </td>
                </tr>
                @endforelse
            </tbody>
