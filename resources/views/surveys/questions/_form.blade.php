...
<div x-data="{
    ...
    <div class="p-6 bg-gradient-to-br from-white to-gray-50 rounded-lg border border-gray-200 shadow-sm">
        ...
        <div>
            ...
            <textarea id="question_body" name="question_body" rows="3" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200" required placeholder="Contoh: Seberapa puaskah Anda dengan layanan kami?">{{ old('question_body', $question->question_body ?? '') }}</textarea>
            ...
            <template x-for="(option, index) in options" :key="index">
                <div class="flex items-center space-x-3 bg-white p-3 rounded-lg border border-gray-200 transition-shadow hover:shadow-md">
                    ...
                    <div class="relative flex-grow">
                        <input type="text" x-bind:name="`options[${index}][option_body]`" x-model="option.option_body" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Teks Opsi Jawaban" required>
                    </div>
                    ...
                    <div class="relative w-32">
                        ...
                        <input type="number" x-bind:name="`options[${index}][option_score]`" x-model="option.option_score" class="w-full pl-10 text-center rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Skor" required>
                    </div>
