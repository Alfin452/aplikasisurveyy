...
@section('content')
    <div class="p-6 bg-white rounded-xl shadow-md animate-fade-in-up">
        <h1 class="text-3xl font-bold text-gray-800 mb-6"><br class="md:hidden">{{ $question->question_body }}</h1>

        <form action="{{ route('surveys.questions.update', [$survey->id, $question->id]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <textarea id="question_body" name="question_body" rows="3" class="w-full rounded-lg border-gray-300 shadow-sm transition duration-300 ease-in-out focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200" required>{{ $question->question_body }}</textarea>
                </div>
                    ...
                    <div id="options-container" class="space-y-4">
                        @foreach ($question->options as $option)
                            <div class="flex items-center space-x-2">
                                <input type="text" name="options[{{ $option->id }}][option_body]" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200" value="{{ $option->option_body }}" placeholder="Teks Opsi Jawaban" required>
                                <input type="number" name="options[{{ $option->id }}][option_score]" class="w-20 rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200" value="{{ $option->option_score }}" placeholder="Skor">
    ...
    <script>
        // Fungsi yang sama dari create.blade.php
        ...
        function addOption() {
            const index = `new_${Math.random().toString(36).substring(2, 9)}`; // ID unik untuk opsi baru
